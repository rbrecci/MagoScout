...
|-----|----------|-----------|
| **Scout** | 1 linha por jogador (01–15) × 9 botões de ação + cronômetro de tempo em quadra + 4 botões coletivos (lateral e escanteio, ofensivo/defensivo) | Registro ao vivo, exige Excel |
...
3. **As macros não vieram.** Os 139 botões chamam `Botao10_1`, `Botao11_2` etc.,
   mas o arquivo é `.xlsx`, que não guarda VBA — não há `vbaProject.bin`. Os
...
eventos são append-only, polling de 3–5s é suficiente para 2 scouts simultâneos.
...
- Aproveitamento sem tentativa nenhuma é `null` e aparece como "—". A linha
